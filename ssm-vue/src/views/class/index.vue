...
<template>
  <div class="app-container">
    <el-form :inline="true" :model="listQuery" class="demo-form-inline">
      <el-form-item label="输入班级名称">
        <el-input v-model="listQuery.name" placeholder="输入班级名称"></el-input>
      </el-form-item>
      <el-form-item>
    ...
    <!--  添加修改页面-->
    <div>
      <el-dialog title="添加/编辑班级信息" :visible.sync="stuVisible">
        <el-form :model="stuform">
          <el-form-item v-for="th in tableHead" :key="th.label" :label="th.label" :label-width="formLabelWidth"
            ...
            <el-input v-if="th.prop !== 'opentime'" v-model="stuform[th.prop]" autocomplete="off" size="small"></el-input>
            <el-date-picker v-else v-model="stuform.opentime" type="datetime" placeholder="选择日期时间" align="right"
  ...
  data() {
    return {
      ...
      tableHead: [
        ...
        { prop: "mambernum", label: "班级人数" },
        { prop: "teacher", label: "班主任" },
        { prop: "opentime", label: "开始时间" },
  ...
  methods: {
    ...
    stuformData() {
      if (
        ...
        //修改方法
        classApi.updateClass(this.stuform).then((response) => {
          this.$message({
            message: response.message,
            type: "success",
          });
        ...
        //添加方法
        classApi.addClass(this.stuform).then((response) => {
          this.$message({
            message: response.message,
            type: "success",
          });
