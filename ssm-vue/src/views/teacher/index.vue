...
<template>
  <div class="app-container">
    ...
    <!--  添加修改页面-->
    <div>
      <el-dialog title="添加/编辑老师信息" :visible.sync="stuVisible">
  ...
  methods: {
    ...
    getDelById(id) {
      this.$confirm("此操作将永久删除该记录, 是否继续?", "提示", {
        ...
        .then(() => {
          return teacherApi.getDelById(id);
        })
        .then((response) => {
          this.$message({
    ...
    stuformData() {
      if (
        ...
        //修改方法
        teacherApi.updateTeacher(this.stuform).then((response) => {
          this.$message({
            message: response.message,
            type: "success",
          });
        ...
        //添加方法
        teacherApi.addTeacher(this.stuform).then((response) => {
          this.$message({
            message: response.message,
            type: "success",
          });
